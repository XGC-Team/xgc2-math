...
void testPose3InertialEskfMidpointIntegrationPublishesCurrentAngularRate() {
    xgc2_math::Pose3InertialEskf eskf;
    const auto imu0 = InertialPoseTestSamples::inertial(1.0, Eigen::Vector3d::Zero());
    eskf.initializeFromPose(InertialPoseTestSamples::pose(1.0, Eigen::Vector3d::Zero()), &imu0);

    const Eigen::Vector3d current_gyro(1.0, -0.4, 0.2);
    const auto imu1 = InertialPoseTestSamples::inertial(1.02, current_gyro);
    eskf.propagateInertial(imu1);

    expect((eskf.state().angular_velocity - current_gyro).norm() < 1.0e-12);
...
void testPose3InertialEskfHighNisWithinAbsoluteGateIsSoftHealth() {
    xgc2_math::Pose3InertialEskfConfig config;
    config.innovation_position_gate_m = 1.0;
    config.pose_position_noise_std = 1.0e-3;
    config.pose_orientation_noise_std = 1.0e-3;
    config.pose_nis_gate = 0.1;
    config.vrpn_health.window_chi_square_gate = 0.1;
    ...
    const auto result = eskf.updatePose(InertialPoseTestSamples::pose(1.01, Eigen::Vector3d(0.05, 0.0, 0.0)));
    expect(result.accepted);
    expect(!result.innovation_rejected);
    expect(result.reject_reason == xgc2_math::PoseFusionRejectReason::kNone);
    expect(result.mahalanobis_distance > config.pose_nis_gate);
    ...
    expect(eskf.vrpnConsecutiveRejects() == 0u);
    expect(eskf.lastFusedPoseStampS() == 1.01);
}

void testPose3InertialEskfPredictionErrorDoesNotRejectContinuousVrpn() {
    xgc2_math::Pose3InertialEskfConfig config;
    config.innovation_position_gate_m = 0.5;
    ...
    auto result = eskf.updatePose(InertialPoseTestSamples::pose(1.01, Eigen::Vector3d(0.1, 0.0, 0.0)));
    expect(result.accepted);

    const auto bad_imu =
        InertialPoseTestSamples::inertial(1.41, Eigen::Vector3d::Zero(), Eigen::Vector3d(80.0, 0.0, 9.8066));
    eskf.propagateInertial(bad_imu);
    expect(eskf.state().position.x() > 0.7);

    result = eskf.updatePose(InertialPoseTestSamples::pose(1.42, Eigen::Vector3d(0.2, 0.0, 0.0)));
    expect(result.accepted);
    expect(!result.innovation_rejected);
    expect(result.reject_reason == xgc2_math::PoseFusionRejectReason::kNone);
    expect(result.position_innovation_norm > config.innovation_position_gate_m);
    expect(eskf.vrpnConsecutiveRejects() == 0u);
    expect(std::fabs(eskf.rawProjectedBodyPose().position.x() - 0.2) < 1.0e-12);
    expect(std::fabs(eskf.lastFusedPoseStampS() - 1.42) < 1.0e-12);
    expect(std::fabs(eskf.state().position.x() - 0.2) < 0.08);
}

void testPose3InertialEskfContinuousVrpnPullsLaggedFilter() {
    xgc2_math::Pose3InertialEskfConfig config;
    ...
    expect(eskf.state().position.x() > 0.15);
    expect(eskf.state().velocity.x() > 0.7);

    const auto held = eskf.updatePose(InertialPoseTestSamples::pose(1.40, Eigen::Vector3d::Zero()));
    expect(held.accepted);
    expect(!held.innovation_rejected);
    expect(held.position_innovation_norm > 0.15);
    expect(held.position_innovation_norm < config.innovation_position_gate_m);
    expect(std::fabs(eskf.state().position.x()) < 0.04);

    for (int k = 1; k <= 8; ++k) {
        eskf.propagateInertial(InertialPoseTestSamples::inertial(1.40 + 0.03 * k, Eigen::Vector3d::Zero(),
                                                                 Eigen::Vector3d(0.0, 0.0, 9.8066)));
        const auto follow = eskf.updatePose(InertialPoseTestSamples::pose(1.40 + 0.03 * k, Eigen::Vector3d::Zero()));
        expect(follow.accepted);
...
void testPose3InertialEskfPosePullsVelocityThroughCrossCovariance() {
    ...
    expect(eskf.covariance()(0, 3) > 1.0e-4);
    const double velocity_before = eskf.state().velocity.x();
    expect(velocity_before > 0.7);

    const auto held = eskf.updatePose(InertialPoseTestSamples::pose(1.40, Eigen::Vector3d::Zero()));
    expect(held.accepted);
    expect(std::fabs(eskf.state().position.x()) < 0.04);
    expect(std::fabs(eskf.state().velocity.x()) < 0.80 * velocity_before);
...
void testPose3InertialEskfGyroBiasAndCorrectedPose() {
    ...
    const auto imu0 = InertialPoseTestSamples::inertial(1.0);
    eskf.initializeFromPose(InertialPoseTestSamples::pose(1.0, Eigen::Vector3d::Zero()), &imu0);
    eskf.propagateInertial(InertialPoseTestSamples::inertial(1.1));
...
void testPose3InertialEskfPoseUpdateRefreshesDerivedImuState() {
    xgc2_math::Pose3InertialEskfConfig config;
    config.pose_position_noise_std = 0.01;
    config.pose_orientation_noise_std = 0.001;
    config.initial_gyro_bias_variance = 0.5;
    config.pose_nis_gate = 1.0e6;
    ...
    const Eigen::Vector3d latest_gyro(0.4, -0.2, 0.1);
    const auto imu1 = InertialPoseTestSamples::inertial(1.1, latest_gyro);
    eskf.propagateInertial(imu1);

    const auto pose = InertialPoseTestSamples::pose(1.1, Eigen::Vector3d::Zero(),
                                                    xgc2_math::rpyToQuaternion(Eigen::Vector3d(0.0, 0.0, 0.2)));
    const auto result = eskf.updatePose(pose);
    expect(result.accepted);
    expect(eskf.state().gyro_bias.norm() > 1.0e-6);
    const Eigen::Vector3d expected_angular_velocity = latest_gyro - eskf.state().gyro_bias;
    expect((eskf.state().angular_velocity - expected_angular_velocity).norm() < 1.0e-12);
}
...
void testPose3InertialEskfVrpnHealthTransitions() {
    ...
    expect(result.vrpn_observation_state != xgc2_math::VrpnObservationState::kFault);
    expect(result.filter_health == xgc2_math::FilterHealth::kDegraded);
    expect(std::fabs(eskf.rawProjectedBodyPose().position.x()) < 1.0e-12);
...
void testTrajectoryAndNmpcProblemContracts() {
    ...
    expect(explicit_planar_samples.setSamples({reverse_a, reverse_b}, true));
...
void testDistanceGjkCoplanarSupportSimplex() {
    ...
    const xgc2_math::SphereSet vehicle(Eigen::Vector3d(6.204626539528, 2.508880552943, 0.0), 0.43);
    const xgc2_math::BoxSet obstacle(Eigen::Vector3d(7.521842, 2.974605, 0.75), Eigen::Vector3d(1.0, 0.7, 1.5),
                                     Eigen::Quaterniond::Identity());
    const xgc2_math::gjk::Result separated =
        xgc2_math::gjk::DistanceGjkQuery::query(vehicle, obstacle, nullptr, 1e-6, 256, 1e-9);
    expect(separated.status == xgc2_math::gjk::Result::Status::kSuccess);
    ...
    expect(std::abs(box_support.support_point.x() - (obstacle.center().x() + 0.5)) < 1e-12);

    const xgc2_math::BallInflatedSet planning_obstacle(std::make_shared<xgc2_math::BoxSet>(obstacle), 0.27);
    const Eigen::Vector3d inflation_direction = Eigen::Vector3d(0.7, -0.2, 0.4).normalized();
    const auto base_inflation_support = obstacle.support(inflation_direction);
    const auto inflated_support = planning_obstacle.support(inflation_direction);
    expect(std::abs(inflated_support.support_value - base_inflation_support.support_value - 0.27) < 1e-12);

    for (const double scale : {0.1, 10.0}) {
        const xgc2_math::SphereSet scaled_vehicle(scale * vehicle.center(), scale * 0.43);
        const xgc2_math::BoxSet scaled_obstacle(scale * obstacle.center(), scale * Eigen::Vector3d(1.0, 0.7, 1.5),
                                                Eigen::Quaterniond::Identity());
        const xgc2_math::gjk::Result scaled = xgc2_math::gjk::DistanceGjkQuery::query(
            scaled_vehicle, scaled_obstacle, nullptr, scale * 1e-6, 256, scale * 1e-9);
        expect(scaled.status == xgc2_math::gjk::Result::Status::kSuccess);
        expect(std::abs(scaled.separator.margin - scale * separated.separator.margin) < scale * 1e-8);
    }
    ...
    const xgc2_math::SphereSet translated_vehicle(vehicle.center() + common_translation, 0.43);
    const xgc2_math::BoxSet translated_obstacle(obstacle.center() + common_translation, Eigen::Vector3d(1.0, 0.7, 1.5),
                                                Eigen::Quaterniond::Identity());
    const xgc2_math::gjk::Result translated =
        xgc2_math::gjk::DistanceGjkQuery::query(translated_vehicle, translated_obstacle, nullptr, 1e-6, 256, 1e-9);
    expect(translated.status == xgc2_math::gjk::Result::Status::kSuccess ||
           translated.status == xgc2_math::gjk::Result::Status::kMaxIterations);
    expect(std::abs(translated.separator.margin - separated.separator.margin) < 5e-4);

    const xgc2_math::SphereSet overlapping(Eigen::Vector3d(6.8, 2.8, 0.0), 0.43);
    const xgc2_math::gjk::Result overlap =
        xgc2_math::gjk::DistanceGjkQuery::query(overlapping, obstacle, nullptr, 1e-6, 256, 1e-9);
    expect(overlap.status == xgc2_math::gjk::Result::Status::kOverlap);
    ...
    for (const double thickness : {1e-10, 1e-13, 1e-16}) {
        ...
        simplex.count = 4;
        simplex.vertices[0].w = Eigen::Vector3d(1.0, 0.0, thickness);
        simplex.vertices[1].w = Eigen::Vector3d(-1.0, 0.0, thickness);
        simplex.vertices[2].w = Eigen::Vector3d(0.0, 1.0, -thickness);
        simplex.vertices[3].w = Eigen::Vector3d(0.0, -1.0, -thickness);
    ...
    for (const double scale : {1e-9, 1.0, 1e9}) {
        for (const double radius : {1e-7, 1e-8}) {
            xgc2_math::gjk::detail::Simplex simplex;
            simplex.count = 4;
            simplex.vertices[0].w = scale * Eigen::Vector3d(1.0, radius, radius);
            simplex.vertices[1].w = scale * Eigen::Vector3d(1.0, -radius, -radius);
            simplex.vertices[2].w = scale * Eigen::Vector3d(-1.0, radius, -radius);
            simplex.vertices[3].w = scale * Eigen::Vector3d(-1.0, -radius, radius);
            for (std::size_t index = 0; index < 4; ++index) {
                simplex.vertices[index].a.setZero();
                simplex.vertices[index].b = simplex.vertices[index].w;
            }
    ...
    const xgc2_math::SupportPointSet origin_point(Eigen::Vector3d::Zero(), {Eigen::Vector3d::Zero()});
    const xgc2_math::SupportPointSet containing_needle(Eigen::Vector3d::Zero(),
                                                       {
    ...
    const xgc2_math::gjk::Result needle_overlap =
        xgc2_math::gjk::DistanceGjkQuery::query(origin_point, containing_needle, nullptr, 1e-8, 256, 1e-14);
...
int main() {
    ...
    testPose3InertialEskfMidpointIntegrationPublishesCurrentAngularRate();
    testPose3InertialEskfTimeJumpResets();
    testPose3InertialEskfPoseUpdateAndReject();
    testPose3InertialEskfHighNisWithinAbsoluteGateIsSoftHealth();
