...
inline void finite(double value) {
    if (!std::isfinite(value)) throw std::invalid_argument("nonfinite wheel model input");
}
inline void positive(double value) {
    finite(value);
    if (value <= 0) throw std::invalid_argument("wheel model parameter must be positive");
}
inline void nonnegative(double value) {
    finite(value);
    if (value < 0) throw std::invalid_argument("negative wheel model parameter");
}
...
inline WheelPair minimizeDisk(double a, double b, double c, double gx, double gy, double radius) {
    ...
    const auto solve = [&](double lambda) {
        ...
        return WheelPair{(-cc * gx + b * gy) / determinant,
                         (b * gx - aa * gy) / determinant};
    };
    WheelPair x = solve(0);
    if (std::hypot(x[0], x[1]) <= radius) return x;
    double lo = 0, hi = std::max({a, c, 1.0});
    for (unsigned k = 0; std::hypot(solve(hi)[0], solve(hi)[1]) > radius; ++k) {
        if (k == 80) throw std::runtime_error("contact projection bracket failed");
        hi *= 2;
    }
    for (unsigned k = 0; k < 70; ++k) {
        const double mid = .5 * (lo + hi);
        x = solve(mid);
        if (std::hypot(x[0], x[1]) > radius) lo = mid;
        else hi = mid;
    }
...
inline ImplicitWheelResult implicitWheelContactStep(const ImplicitWheelInput& in,
                                                   const ImplicitWheelMemory& old,
                                                   const ImplicitWheelParameters& p = {}) {
    using namespace implicit_wheel_detail;
    positive(in.dt_s); positive(p.mass_kg); positive(p.yaw_inertia_kg_m2);
    positive(p.motor_p_nm_s); positive(p.motor_i_nm); positive(p.motor_limit_nm);
    positive(p.kkt_tolerance); nonnegative(p.passive_drag_nm_s);
    if (p.max_sweeps == 0) throw std::invalid_argument("zero contact iterations");
    ...
    for (std::size_t i = 0; i < 4; ++i) {
        positive(p.wheel_inertia[i]); positive(p.stiffness_n_m[i]);
        nonnegative(p.friction[i]); nonnegative(p.renewal_length_m[i]);
        nonnegative(in.normal_force_n[i]); positive(in.radius_m[i]);
        finite(in.target_rad_s[i]); finite(old.drive_error_rad[i]);
        const auto r = in.contact_offset_world_m[i];
        const auto t = in.rolling_direction_world[i];
        for (double v : r) finite(v);
        for (double v : t) finite(v);
        for (double v : old.tread_m[i]) finite(v);
        ...
        columns[8 + i][3 + i] = 1;
        const double gamma = p.renewal_length_m[i] > 0
            ? std::abs(in.radius_m[i] * in.velocity[3 + i]) / p.renewal_length_m[i] : 0;
        diagonal[2 * i] = diagonal[2 * i + 1] =
            (1 + h * gamma) / (h * h * p.stiffness_n_m[i]);
    ...
    for (std::size_t j = 0; j < 7; ++j) {
        finite(in.velocity[j]); finite(in.external_force[j]);
    ...
    for (std::size_t a = 0; a < 12; ++a) {
        for (std::size_t j = 0; j < 7; ++j) linear[a] += columns[a][j] * free[j];
        finite(linear[a]); positive(diagonal[a]); finite(limit[a]);
    ...
    for (unsigned sweep = 1; sweep <= p.max_sweeps; ++sweep) {
        for (std::size_t i = 0; i < 4; ++i) {
            ...
            for (std::size_t j = 0; j < 12; ++j) if (j != a && j != a + 1) {
                gx += matrix[a][j] * impulse[j];
                gy += matrix[a + 1][j] * impulse[j];
            }
            const auto x = minimizeDisk(matrix[a][a], matrix[a][a + 1], matrix[a + 1][a + 1],
                                        gx, gy, limit[a]);
            impulse[a] = x[0]; impulse[a + 1] = x[1];
        }
        for (std::size_t a = 8; a < 12; ++a) {
            double g = linear[a];
            for (std::size_t j = 0; j < 12; ++j) if (j != a) g += matrix[a][j] * impulse[j];
            impulse[a] = std::clamp(-g / matrix[a][a], -limit[a], limit[a]);
        }
        WheelVector12 projected{};
        for (std::size_t a = 0; a < 12; ++a) {
            double gradient = linear[a];
            for (std::size_t j = 0; j < 12; ++j) gradient += matrix[a][j] * impulse[j];
            projected[a] = impulse[a] - gradient / lipschitz;
        }
        for (std::size_t i = 0; i < 4; ++i) {
            const auto x = disk({projected[2 * i], projected[2 * i + 1]}, limit[2 * i]);
            projected[2 * i] = x[0]; projected[2 * i + 1] = x[1];
            projected[8 + i] = std::clamp(projected[8 + i], -limit[8 + i], limit[8 + i]);
        }
        result.kkt_residual = 0;
        for (std::size_t a = 0; a < 12; ++a)
            result.kkt_residual = std::max(result.kkt_residual,
                                           lipschitz * std::abs(projected[a] - impulse[a]));
        result.sweeps = sweep;
        if (result.kkt_residual <= p.kkt_tolerance * scale) break;
        if (sweep == p.max_sweeps) throw std::runtime_error("coupled wheel solve did not converge");
    }
    ...
    for (double v : result.predicted_velocity) finite(v);
    return result;
}
